...
- [ ] 1.1 Create `client/src/levels/world.ts`: `isCaseTrail(level)` (`!!level.clue`, `LevelPlay.tsx:633`'s old logic under its real name) and `inDetectiveWorld(level)` (`isCaseTrail(level) || level.detectiveWorld === true` — widening only).
- [ ] 1.2 Add `detectiveWorld?: boolean` to `LevelConfig` in `client/src/levels/types.ts`.
