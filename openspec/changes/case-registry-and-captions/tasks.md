...
- [x] 2.15 **Screenshot check (human-reviewed), the flagged risk.** PASS — `/tmp/shots/webfoot-drained-on-earth.png` (direct composite of `clue-webfoot-drained.png` on `CORRIDOR_EARTH #d9c3ae`) shows the flat `#c8cdd2` silhouette clearly legible against the earth tone via hue contrast (cool grey vs warm tan), even though the two colours' luma is nearly identical (204 vs 199). Not the lamp's near-invisible failure; no grey companion source needed. *(Superseded: `CLUE_DRAINED` later moved to `#838383` and the rule became luma separation from the ground rather than hue contrast. The check stands as the record of what was measured then.)*
