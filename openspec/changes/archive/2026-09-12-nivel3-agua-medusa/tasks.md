...
- [x] 5.1 Add two `SINGLES` rows to `scripts/art/build_art.py`: `('medusa.png', 'goal-medusa.png', 384, None, True)` and `('estrella de mar.png', 'hazard-starfish.png', 320, None, True)` — `fill=None`, no new palette tuple.
- [x] 5.2 **Run `python3 scripts/art/build_art.py` and commit its outputs**, including `client/public/art/goal-medusa.png`, `client/public/art/hazard-starfish.png`, and the regenerated `client/public/art/manifest.json`, in this same commit. Art never enters by hand. `ART_OUTLINE` is the token for every drawn-world contour.
- [x] 5.3 Read the freshly emitted `manifest.json` for the two new entries' `w`/`h`; add `GOAL_MEDUSA_ART` and `HAZARD_STARFISH_ART` to `client/src/detective/assets.ts` using those emitted sizes, never guessed.
- [x] 5.4 Update `client/src/detective/artManifest.test.ts`: `REGISTERED` gains `['GOAL_MEDUSA_ART', GOAL_MEDUSA_ART]` and `['HAZARD_STARFISH_ART', HAZARD_STARFISH_ART]` plus their imports (`:26-37`, `:68-82`); `:122` count `44 → 46`; `:117-118` comment gains "+ 1 goal (medusa) + 1 hazard (estrella de mar)".
- [x] 5.5 **Screenshot check (human-reviewed, mandatory — contour colour). Performed — FAILED, not silently accepted; see apply-progress.md.** Directly measured (`png.py`, not eyeballed) both emitted PNGs' authored contour: near-fully-saturated navy (`goal-medusa.png` ≈ rgb(0,18,120), chroma ≈118/255; `hazard-starfish.png` ≈ rgb(0,20,122), chroma ≈122/255), not the near-achromatic `ART_OUTLINE` (`#1a1a1a`, chroma 0). This is FAIL by the task's own criterion, and roughly 10x more saturated than the already-flagged `#19241c` grass failure (chroma ≈11). **Not resolved in this apply pass**: the task's prescribed fallback ("move to a two-tone recolour") requires choosing a specific `fill` RGB per creature, which is a creative/art-direction decision outside this executor's authority to invent unilaterally — reported per the launch prompt's explicit instruction to "say so plainly rather than shipping a third coloured line into the drawn world" instead of silently applying an invented recolour. Left `fill=None` (5.1's assignment) in place pending an explicit decision.
- [x] 5.6 Run `npm test` and `npm run build`; confirm green.
...
- [x] 7.6 **Screenshot check (human-reviewed) — microprogression.** Start the dev server; `scripts/shot.sh http://localhost:<port>/?nivel=f2-guirnalda /tmp/shots/nivel3-1-guirnalda.png`, then the same for `f2-agua2`, `f2-agua3`, `f2-agua4` into `/tmp/shots/nivel3-2-agua2.png`, `nivel3-3-agua3.png`, `nivel3-4-agua4.png`. PASS: viewed in id order, corridor width visibly narrows 1→2, desafío 3 reads as five differently-sized U's (not a mistake), desafío 4 returns to desafío 1's wide geometry. FAIL: two adjacent shots look identical, or desafío 3's U's look uniform. **Performed — PASS, see apply-progress.md.**
- [x] 7.7 **Screenshot check (human-reviewed) — desafío 3's ideal band.** Inspect `nivel3-3-agua3.png` (crop closer if needed) at the shallowest U (the `{130, 95}` cycle). PASS: the pale ideal-band fill hugs the U bottom with a small but real margin, consistent with the computed 4.5-unit margin — the tightest of the four levels. FAIL: the band appears to bulge past the U's stroke or looks pinched/folded at that bottom. **Performed — PASS, with a discrepancy noted: the 4.5-unit margin actually belongs to the `{165,170}` cycle, not `{130,95}`; both were inspected. See apply-progress.md.**
- [x] 7.8 **Screenshot check (human-reviewed) — medusa at the goal.** Inspect any of the four shots' route end. PASS: the medusa (`goal-medusa.png`) stands at the route's end, not the two hollow diamonds and not a case lamp. FAIL: the endpoint shows the default diamonds, the lamp, or no art. **Performed — PASS, see apply-progress.md.**
- [x] 7.9 **Screenshot check (human-reviewed) — starfish crossing the path.** Inspect `nivel3-4-agua4.png`. PASS: the starfish art visibly occupies/crosses the middle-U corridor, drawn as `hazard-starfish.png`, not a bare circle. FAIL: the hazard renders as a plain circle, is missing, or sits outside the corridor. **Performed — the named screenshot alone landed in the hazard's intended "gap" phase (off the corridor); a second, deliberately re-timed capture confirms it crossing the corridor as `hazard-starfish.png`, not a circle. See apply-progress.md for the full honest account.**
- [x] 7.10 Run `npm test` and `npm run build`; confirm green.
