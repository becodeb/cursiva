...
### Requirement: buildCombination Invariants → buildWord Invariants

(Reason: decision 1 — free word building replaces fixed COMBO_REGISTRY pairs; `buildCombination` becomes the 2-letter case of `buildWord`.)
(Migration: update caller/tests to `buildWord`; `combinations.test.ts` pair assertions replaced.)
...
(Reason: decision 1 — free word building replaces COMBO_REGISTRY and the 12-pair picker (US4).)
(Migration: drop `COMBO_REGISTRY` from `registry.ts`; pair-selection tests replaced by `wordBuilding.test.ts`; reversed-pair distinctness no longer applies.)