...
| U7 | Fogged glass polish | `RevealLayer.tsx`, reveal-grid tests, glass levels in `catalog.ts` | Start/partial/success glass captures. |
| U8 | Night discovery polish | `catalog.ts`, `backdrops.ts`, `RevealLayer.tsx`, `LevelPlay.test.tsx` | Night start/partial/error/success captures. |
...
| U10 | Narrative closures | `zoo/adventures.ts`, `AdventureIntro.tsx`, `AdventureClosing.tsx`, `GameScreen.test.tsx` | Approved closings render; jellyfish/medusa gaps isolated. |
...
| U13 | Visual QA | Playwright evidence paths | Fresh QA verifies each applicable visual unit has 15 real-scene cells and covers map, glass, night, PISTAS, closing, backgrounds, and U14 signage. |
| U14 | docs/16 sign-frame defect | In-level `CaptionedArt` rendering/tests | Add missing wooden zoo-sign frame around in-level art, not only Pulpito speech bubbles; preserve caption/image semantics; stay under 400 changed lines. |
...
| E2E | Required visual states | Playwright matrix: viewports `1280x720`, `844x390`, `390x844`; states start, partial, error, success, map-return; cover map, glass, night, PISTAS, approved narrative closing, renewed backgrounds, and the docs/16 in-level sign frame, with 15 cells per applicable visual unit. |
