...
| ID | Task | Branch | Route | Status | Evidence |
|---|---|---|---|---|---|
| T7 | Level screen: the backdrop art fills 100% of the viewport (the playable sheet may shrink relative to the art; nothing clipped at 1024x768, 1180x820, 768x1024, 844x390); chrome floats over the art in the game's marker style (`docs/09`); drop "repeat"; auto-advance after the success celebration, tap to skip | `feat/fullscreen-level-ui` | delegated (writer trigger) | in progress | First pass (`a9af067`) shipped a `.cv-backdrop-fill` layer behind the unchanged sheet — orchestrator review of the screenshots found it draws the art TWICE (picture-in-picture: duplicated reed bands on duck-trail1, mismatched rock/fence seams at the sheet edges on snake1). Reworking to an expanded-viewBox approach instead: the SVG itself grows to fill the viewport, one continuous backdrop image, no second layer. See this row's evidence once the rework lands. |
...
- 2026-09-25: T7 first pass (`a9af067`) rejected by orchestrator review — the `.cv-backdrop-fill` approach drew the art twice (picture-in-picture, mismatched seams). Reworking to an expanded-viewBox approach: merged `main` (T8+T9) into `feat/fullscreen-level-ui` and restarted from the SVG-fill design. See the T7 row's own evidence once it lands.
