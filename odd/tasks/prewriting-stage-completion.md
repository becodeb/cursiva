...
| ID | Task | Branch | Route | Status | Evidence |
|---|---|---|---|---|---|
| T7 | Level screen: the backdrop art fills 100% of the viewport (the playable sheet may shrink relative to the art; nothing clipped at 1024x768, 1180x820, 768x1024, 844x390); chrome floats over the art in the game's marker style (`docs/09`); drop "repeat"; auto-advance after the success celebration, tap to skip | `feat/fullscreen-level-ui` | delegated (writer trigger) | done | `a9af067` — a new `.cv-backdrop-fill` layer (`position: fixed; inset: 0; object-fit: cover`) renders the level's own adventure backdrop art full-viewport behind every layer of chrome, for every family with a backdrop (reveal/cleaning, night, snakes, dolphins, duck, sheep, bee, turtles, monkeys, hedgehog — verified by screenshot, see below); `.cv-sheet`'s own sizing is untouched (the sheet did not need to shrink). `.cv-btn`/`.cv-result-pill` restyled to the marker look (`background: ${SHEET_PAPER}`, `border: 3px solid #1a1a1a`, replacing the flat white/`#cbd5e1` pair) — this covers back, demo/play, and (on a classic level only) Borrar/Siguiente, since they all share `.cv-btn`. The repeat ("Borrar") button is gone outright on every drawn-place level; a fresh touch after a failed attempt now clears the old ink itself (`clearOnFailedRetryRef`, scoped to `level.kind === 'path'` — a `free` level's strokes are cumulative wanted progress, never cleared, and confirmed safe: `mustBeContinuous: false` makes `extraLifts` always 0, so fluency can never get a free-kind child stuck). Auto-advance: an approved drawn-place attempt starts a `CELEBRATE_MS` (1800ms) timer that calls the SAME `onNext` the old "Siguiente" button used; a full-screen transparent `.cv-celebrate-skip` button (guarded by a 400ms grace window against the winning stroke's own release synthesizing a click) lets a tap skip the wait. Tests: `client/src/screen/LevelPlay.test.tsx`, 138 passed (was 133) — new T7 describe block asserts the backdrop image's presence/absence, the marker-style CSS facts, the repeat button's removal, and the celebration markup's absence before any attempt (the approved-and-celebrating render is real-browser-only, per this file's own established convention — proven instead by the browser trace below). Checks: `npx vitest run --maxWorkers=2 src/screen/LevelPlay.test.tsx` → 138 passed; full suite `npx vitest run --maxWorkers=2` → 2179 passed (87 files); `npm run build` → clean; `git diff --check` → clean. Browser QA (system Chromium, `--disable-gpu`, dev server on port 5197): screenshots of glass1/sand3/night2/snake1/dolphin1/duck-trail1/sheep-hill1/bee1/turtle1/monkey1/hedgehog1 at 1024x768 and 844x390, plus glass1/duck-trail1 at 1180x820 and 768x1024 — nothing clipped at any tier, `capturas/2026-09-25-tanda2/*.png` (git-ignored). Traced glass1 to a real approval with one continuous simulated stroke: `trace-02-celebrating.png` shows the marker-style "✓ ¡Vidrio limpio!" pill; with NO further pointer input, the screen auto-advanced past the CELEBRATE_MS window with no manual tap (`trace-04-after-auto-advance.png` — the adventure's own closing screen, since `peces` is a single-level enclosure adventure). Failed-attempt retry proven on duck-trail1: a deliberately bad stroke leaves faint off-corridor ink (`retry-01-after-failed-attempt.png`, no Borrar/Siguiente present), and the next touch clears it before drawing resumes (`retry-02-fresh-touch-clears.png`) — a child can always retry. Not changed, flagged instead: `client/src/voice/SpeakButton.tsx` (the "hear it again"/sound button) is still a plain white circle — it is shared, byte-for-byte, across five screens (prologue/intro/closing/map/level) with an explicit cross-screen consistency contract in its own header, sits outside this task's declared files, and restyling it would also reach the parallel `feat/character-life-animations` (T8) writer's screens; left for the user/orchestrator to decide whether it is worth a follow-up task. `capturas/2026-09-25-tanda2/screenshots.js`/`trace-glass1.js`/`trace-retry.js` are the throwaway QA scripts (git-ignored alongside the screenshots). |
...
- 2026-09-25: T7 done on `feat/fullscreen-level-ui` (`a9af067`), resumed after a container restart killed the previous writer mid-task (uncommitted work in the worktree was reviewed, kept, and finished — see the T7 row's own evidence). Not merged to `main` yet; T8/T9 (the other two batch 2 writers) still open.
- Open from T7 for the user to decide: `SpeakButton.tsx` (the sound/"hear it again" button) still reads as a generic white circle — deliberately not restyled, see the T7 row's own note on why.
