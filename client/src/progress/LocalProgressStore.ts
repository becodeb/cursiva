...
// Design-fixed semantics: values CLAMP to 0–100 and ROUND to whole
// percentages (every spec scenario uses integer values), letters stay
// ISOLATED, and registration is MONOTONIC best-of `max(stored, round(clamp))`
// — mastery, not last attempt (design.md Progress % row). Rounding makes
// exact mastery reachable: a perfect trace scores ~99.99 (float CTM noise)
// and must persist as exactly 100 or the family bloom could never trigger.
// A corrupt payload reads as empty and is overwritten on the next write; a
// throwing storage (privacy mode) degrades to in-memory storage and NEVER
// throws.
...
const norm = (v: number): number => Math.round(v < 0 ? 0 : v > 100 ? 100 : v)
...
export class LocalProgressStore implements ProgressStore {
  ...
  private load(): void {
    ...
    try {
      ...
      for (const [id, value] of Object.entries(parsed)) {
        ...
        if (Number.isFinite(n)) this.memory.set(id, norm(n))
      }
    } catch {
  ...
  setProgress(letterId: string, value: number): void {
    const next = Math.max(this.getProgress(letterId), norm(value)) // monotonic best-of
