 ...
 *
 * The migration is idempotent by construction — a destination id that already
 * carries a record is never touched again — so calling this on every visit to
 * the home costs one storage read and changes nothing on the second call.
 */
export function openProgressStore(): LevelProgressStore {
  const store = new LevelProgressStore()
  const migrated = migratePhase1(store.all())
