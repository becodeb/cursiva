...
| D3 | El Pulpito aparece dos veces en la primera lámina (chiquito adentro del globo y grande abajo) | baja | arte/decisión |
...
| D15 | El barro del sendero es un rectángulo marrón plano (`antes-05`) y al terminar dice "¡Vidrio limpio!" | media | T3 (frase) + P13 (dibujo) |
...
| D21 | Al terminar un tramo no hay festejo: solo cambia de color un botón chico abajo | media | T3 (pulso) + T6 |
| D22 | A 844×390 la barra de PISTAS se come el alto y la hoja queda en 383×230 (`antes-11`) | media | pendiente P3 |
| D23 | En `snake1` el Pulpito tapa la cabeza de la primera víbora | baja | pendiente |
| D24 | Abeja: sin demo ni consigna; flor gris; no se entiende qué hacer | media | T7 + pendiente P5 |
...
|---|---|---|
| **Entrada** | El recinto vacío y el Pulpito: "¡El pato no está! Mirá: dejó huellas. ¿Las seguimos?" | existe (texto); voz en T7 |
| **Demo** | La línea recorre el camino. "Mirá cómo va… ¡ahora vos!" | existe; voz en T7 |
| **Tramos 1…N−1** | Cada nivel es un pedazo del camino del animal. Al final hay una **pista** (pluma, miga, burbuja…). Al llegar, la pista vuela a la barra de arriba. "¡Una pluma! Pasó por acá. ¡Sigamos!" — y el tramo siguiente arranca sin pasar por el mapa | flujo en T2; barra en T6 |
| **Encuentro** | El último tramo termina en el animal | T6 (silueta en la barra); arte del animal en la meta, P4 |
| **Cierre** | Festejo corto con el animal grande: "¡Encontramos al pato! Ya está en su laguna." | T8 |
| **Mapa** | El pato está en su laguna, se levanta la niebla del próximo sector y el foco apunta ahí | T4 (foco); animación de llegada, P6 |
...
| Tarea | Arregla | Estado |
|---|---|---|
| T1 — Un nivel por recinto en la entrada (`glass1`, `sand1`, `glass3`, `sand3`; los otros cuatro ids quedan en el catálogo) | D11 | en curso |
| T2 — La aventura se juega de corrido; el mapa al final | D12, 1 | en curso |
| T3 — La hoja no se achica; sin margen de 8 px; "¡Sendero limpio!" en el barro; "Siguiente" que late cuando se habilita | D2, D13, D15, D21 | pendiente |
| T4 — Mapa: foco sobre el destino, globo que no lo tapa, se cierra y se reabre tocando al Pulpito; mapa centrado; sector terminado se rejuega desde el principio | D4, D5, D7, D8, D10 | pendiente |
| T5 — El cartel del recinto sale del dibujo | D14 | pendiente |
| T6 — La barra de pistas muestra al animal buscado y el avance | D19, D20 | pendiente |
| T7 — Narrador con voz | D1, D24, D26 | pendiente |
| T8 — Cierre con rescate para cada animal | D27, D28 | pendiente |
...
| P2 | Álbum de animales (§4.7.3) | La meta larga; hace visible "juntar animales" | código + arte |
| P3 | Celular horizontal: la barra de arriba pasa a una columna lateral cuando la altura es chica (D22) | En 844×390 la hoja queda en un 45 % del ancho | código |
| P4 | El animal (o su silueta) como meta del último tramo, en vez del rombo | Cierra el "lo encontramos" dentro del juego | código |
| P5 | Demo y consigna hablada en abeja y noche (D24) | Hoy no se entiende qué hacer | código |
| P6 | Llegada del animal al mapa con animación, la primera vez | "Recuperar" se ve | código |
| P7 | Explicar o sacar la segunda estrella (D29) | Hoy pide repetir sin decir por qué | decisión |
| P8 | Voz grabada que reemplace a la voz sintética (el narrador ya tiene un registro por frase) | La voz del navegador es genérica | audio |
| P9 | Aventuras de peces (reusar medusa), monos (bucles) y tortugas (óvalos) — §4.5 | Cumple la promesa y cubre los patrones que faltan | guion + código + arte |
| P10 | Arte A1–A5 | Ver §6 | arte |
...
| P11 | Actualizar las specs de OpenSpec (entrada de 4 ids, flujo por aventura) | Hoy contradicen el código | U15.1 |
