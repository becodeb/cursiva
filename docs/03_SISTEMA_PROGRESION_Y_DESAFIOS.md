...
El interruptor real no es el número de nivel: es la pertenencia al **mundo** del detective, `inDetectiveWorld` en `client/src/levels/world.ts`. Hasta el cambio de Nivel 3 esta frase decía `isDetectiveTrail`, `!!level.clue`, porque llevar una pista y vivir en el mundo dibujado eran la misma pregunta — nunca había un nivel que hiciera una cosa sin la otra. Nivel 3 es el primero que las separa: pasto, tierra pisada, el pulpo parado con la lupa, y ninguna pista propia. Por eso el flag se partió en dos — `isCaseTrail` (`!!level.clue`, la pregunta original, intacta) para lo que pertenece a un CASO: el riel PISTAS, el farol; e `inDetectiveWorld` (`isCaseTrail(level) || level.detectiveWorld === true`) para lo que pertenece al MUNDO: el suelo, la tinta de barro, el pulpo, la cáscara sin palabras y, con ella, el bloque de resultado donde viven las tres estrellas. Un rastro de detective siempre está en el mundo; la vuelta ya no es cierta, y Nivel 3 es la prueba.

Los ocho senderos del Nivel 2, los cuatro del pato y los cuatro de la gallina, llevan pista y por eso son `isCaseTrail`. Los cuatro desafíos del Nivel 3 no llevan pista —no hay nada que recoger, no hay riel, no hay deducción, la medusa es sencillamente la meta— pero sí llevan `detectiveWorld: true`, así que también son `inDetectiveWorld` y también montan la cáscara sin palabras. No es un descuido ni un defecto: la cáscara sin palabras es una decisión tomada y con tests que la fijan. Pero el efecto es que **las estrellas aparecen recién cuando el chico sale del mundo dibujado**, y eso es información pedagógica, no un detalle de implementación. El Nivel 1 no las muestra porque no exige precisión; los Niveles 2 y 3 no las muestran por la misma razón entre sí —su pantalla no tiene bloque de resultado—, aunque lleguen a esa pantalla por caminos distintos: el Nivel 2 porque cada rastro lleva pista, el Nivel 3 porque vive en el mundo sin llevarla. Tres razones distintas que dan el mismo resultado, y sólo coinciden por ahora.
